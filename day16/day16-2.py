...
def addTopConfig(mapArray, initialBeamArray):
    for i in range(len(mapArray[0])):
        initialBeamArray.append(Beam(0, i, "down"))

def addBottomConfig(mapArray, initialBeamArray):
    bottomRow = len(mapArray) - 1
    for i in range(len(mapArray[bottomRow])):
        initialBeamArray.append(Beam(bottomRow, i, "up"))

def addLeftConfig(mapArray, initialBeamArray):
    for i in range(len(mapArray)):
        initialBeamArray.append(Beam(i, 0, "right"))

def addRightConfig(mapArray, initialBeamArray):
    rightCol = len(mapArray[0]) - 1
    for i in range(len(mapArray)):
        initialBeamArray.append(Beam(i, rightCol, "left"))
...
with open(file_path, 'r') as file:
    ...
    maxEnergisedCount = 0
    initialBeamArray = []
    addTopConfig(mapArray, initialBeamArray)
    addBottomConfig(mapArray, initialBeamArray)
    addLeftConfig(mapArray, initialBeamArray)
    addRightConfig(mapArray,initialBeamArray)

    for initialBeam in initialBeamArray:
        beamQueue = queue()
        ...
        maxEnergisedCount = max(maxEnergisedCount, energisedCount)

    print(maxEnergisedCount)
